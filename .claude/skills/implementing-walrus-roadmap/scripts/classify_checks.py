...
walrus specifics this encodes:
  * CI fires on BOTH `push` and `pull_request`, so every check name appears
    twice on a PR head. Duplicates are irrelevant to the verdict (both copies
    must pass) but are counted so a caller can tell a half-registered head from
    a genuinely single run.
