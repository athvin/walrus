...
async fn full_status_walk_and_duplicate_request_rejected() {
    ...
    // Claim: requested → exporting, lease attached. `customers` is a resync, so only `orders`
    // shows up as an active REBUILD (the loader-pause input).
    let claimed = reload::claim_requested(&mut *tx, epoch, "sink-a", 60, 10)
        .await
        .unwrap();
    assert_eq!(claimed.len(), 2, "both requested rows claimed");
    let orders = claimed.iter().find(|r| r.reload_id == id).unwrap();
    ...
    // Nothing left in `requested`: a raced second claimer gets an empty Vec, not an error.
    let raced = reload::claim_requested(&mut *tx, epoch, "sink-b", 60, 10)
        .await
        .unwrap();
    assert!(raced.is_empty());

    // Only the exporting `reload`-flavor row is a rebuild; the resync never pauses anything.
    ...
    // The holder renews; a phantom does not.
    assert!(reload::renew_lease(&mut *tx, id, "sink-a", 60)
        .await
        .unwrap());
    ...
    // Chunk 1 freezes L₁ + schema_version; chunk 2's different values must NOT overwrite them.
    let l1: Lsn = "0/100".parse().unwrap();
    let l2: Lsn = "0/200".parse().unwrap();
    reload::advance_cursor(&mut *tx, id, 1, &serde_json::json!([42]), l1, 7)
        .await
        .unwrap();
    reload::advance_cursor(&mut *tx, id, 2, &serde_json::json!([84]), l2, 9)
        .await
        .unwrap();
    let row = reload::get(&mut *tx, id).await.unwrap().unwrap();
    assert_eq!(row.chunk_no, 2);
