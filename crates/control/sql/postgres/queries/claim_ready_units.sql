WITH gate AS MATERIALIZED (
    ...
    WHERE NOT EXISTS (
      ...
      AND NOT EXISTS (
        ...
        WHERE recovery.epoch = $1
          AND recovery.source_schema = $2
          AND recovery.source_table = $3
          AND recovery.status IN ('retrying', 'quarantined')
    )
), group_parents AS MATERIALIZED (
    -- Parents are authoritative. Active parents remain work even with no/malformed children; a
    -- terminal parent retaining children is corruption and must block later work too.
    SELECT parent.*
    FROM gate
    JOIN walrus.stream_manifest_group parent
      ON parent.epoch = $1
    ...
    SELECT manifest.*
    FROM gate
    JOIN walrus.file_manifest manifest
      ON manifest.epoch = $1
...
ORDER BY work_lsn, work_kind, work_id, id
