...
//! the same file appends **zero** rows. The fixture Parquet is written by DuckDB itself (`COPY … TO
//! 's3://…'`), so this test needs no sink dependency.
//!
...
/// Write a fixture Parquet (2 orders rows + a JSON meta column) to MinIO and return its s3 URI.
...
async fn seed_manifest(pool: &sqlx::PgPool, epoch: i64, uri: &str) -> i64 {
...
async fn setup(epoch: i64) -> (sqlx::PgPool, TableDb, std::path::PathBuf) {
...
fn ctx<'a>(pool: &'a sqlx::PgPool, epoch: i64, db: &'a TableDb) -> TableCtx<'a> {
    TableCtx {
        pool,
        epoch,
        schema: "public".into(),
...
async fn appends_rows_verbatim_with_promoted_columns_and_meta_intact() {
    ...
    let (pool, db, dir) = setup(epoch).await;
    seed_manifest(&pool, epoch, &uri).await;

    let lsn = run_phase_a(&ctx(&pool, epoch, &db)).await.unwrap();
    assert_eq!(lsn, Some("0/64".parse().unwrap()));
    assert_eq!(raw_count(&db), 2, "both rows appended verbatim");

    // A specific value — not just a count — to catch a silent column shift.
    let (op, meta, promoted_lsn): (String, String, String) = db
        .conn()
        .query_row(
...
async fn advances_raw_watermark_and_deletes_the_claimed_manifest_rows() {
    ...
    run_phase_a(&ctx(&pool, epoch, &db)).await.unwrap();
    ...
    let remaining = control::claim_ready(&pool, epoch, "public", "orders", 100)
        .await
        .unwrap();
    assert!(
        remaining.is_empty(),
    ...
    // The mirror is untouched by Phase A.
    let mirror: i64 = db
        .conn()
...
async fn re_running_the_same_file_appends_zero_rows() {
    ...
    seed_manifest(&pool, epoch, &uri).await;
    run_phase_a(&ctx(&pool, epoch, &db)).await.unwrap();
    assert_eq!(raw_count(&db), 2);

    // Crash-window replay: the file is re-claimed (a fresh manifest row) and re-appended.
    seed_manifest(&pool, epoch, &uri).await;
    run_phase_a(&ctx(&pool, epoch, &db)).await.unwrap();
    assert_eq!(
        raw_count(&db),
