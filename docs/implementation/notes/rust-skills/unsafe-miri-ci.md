...
There are 0 first-party unsafe syntax sites across the 196 Rust files under `crates/` and `tests/`.
The two plain-word `unsafe` matches are documentation about unsafe DDL casts, not Rust syntax.
`Cargo.toml:16` sets `unsafe_code = "forbid"` in `[workspace.lints.rust]`, and every one of the six
...
forms (`Cargo.toml:18-28`). Those static policies make growth of first-party unsafe code a build
...
`control` exercise real TCP and Postgres paths. The current tests contain 131 `#[ignore...]`
compose-gated cases behind 136 `#[tokio::test]` attributes, so Miri's isolation model cannot turn
the integration suite into coverage merely by making it slower. This is why a future Miri run must
start with the specific crate and unit tests that own new first-party unsafe code.
...
dedicated MSRV job at `.github/workflows/ci.yml:528-553` prevents that pin and the declared MSRV
...
`Cargo.toml:142-147` adopts thin LTO but rejects `codegen-units = 1` because a few percent more
...
CI already runs the script before formatting, Clippy, and tests. There is deliberately no Miri
...
(`crates/common/src/config_test.rs:19-30`), while its metrics tests install a process-wide
