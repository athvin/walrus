...
The same relocation as PR 7.1, applied to `pg-to-arrow`'s **9** files with inline test modules. This
crate holds the two largest test modules in the sweep — `batch.rs` (~480 lines of RecordBatch-builder
assertions) and `schema.rs` (~269 lines) — so the payoff in source-file readability is the biggest
here. The convention was set by 7.1; this PR just executes it.
...
- Convert the **9** `pg-to-arrow` files with an inline `#[cfg(test)] mod tests { … }` —
  ...
  `tier2.rs`, `uuid_enum.rs` — to `#[cfg(test)] mod tests;` + a sibling `src/<module>/tests.rs`.
...
```
crates/pg-to-arrow/src/{batch,schema,descriptor,range,parquet,geometric,tier3,tier2,uuid_enum}.rs        # modify — inline block → `mod tests;`
crates/pg-to-arrow/src/{batch,schema,descriptor,range,parquet,geometric,tier3,tier2,uuid_enum}/tests.rs  # new — moved bodies
```

## Skeleton

```rust
// AFTER — crates/pg-to-arrow/src/batch.rs (tail): the ~480-line block becomes one line
#[cfg(test)]
mod tests;
```

```rust
// AFTER — crates/pg-to-arrow/src/batch/tests.rs (new): the moved body
use super::*;
...
- [ ] All 9 files carry `#[cfg(test)] mod tests;` with the body moved to `src/<module>/tests.rs`; no
      `mod tests {` brace-block remains in `pg-to-arrow`.
...
0
$ find crates/pg-to-arrow -name tests.rs | wc -l
9
$ wc -l crates/pg-to-arrow/src/batch.rs   # was ~1360, now ~880 (production surface only)
```

`batch.rs` reads as builder code; `batch/tests.rs` reads as its assertions. Same green suite.
